...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (productImages.length > 0 && lightboxModal && lightboxImg) {
        productImages.forEach(wrapper => {
            wrapper.addEventListener('click', () => {
                const img = wrapper.querySelector('img');
                const title = wrapper.closest('.product-card').querySelector('.product-title').innerText;
                const category = wrapper.closest('.product-card').querySelector('.product-badge').innerText;
                
                lightboxImg.src = img.src;
                lightboxCaption.innerText = title;
                lightboxSubcaption.innerText = category;
    ...
    // 8. Auto-fill Quote Request in Contact Form
    const urlParams = new URLSearchParams(window.location.search);
    const productParam = urlParams.get('product');
    
    if (productParam && contactForm) {
        const decodedProductName = decodeURIComponent(productParam).replace(/-/g, ' ');
        const formSubject = document.getElementById('formSubject');
        const formMessage = document.getElementById('formMessage');

        if (formSubject) {
            formSubject.value = 'wholesale';
        }
        
        if (formMessage) {
            formMessage.value = `Hello Bhagwati Mandap Bazaar,\n\nI am interested in receiving a wholesale quote, sizing options, and delivery timeline for the product: "${decodedProductName}".\n\nPlease let me know the pricing details and fabrication timeline.\n\nThank you!`;
        }
